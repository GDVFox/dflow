...
func HandleSchemas(rawArgs []string) {
	if len(rawArgs) < 2 {
		pterm.Error.Printfln("Expected COMMAND, run 'dflow %s help' for more information", metaclient.MetaNodeAddress)
		return
	}
	args := rawArgs[1:]
	...
	default:
		pterm.Error.Printfln("Unknown command '%s', run 'dflow %s help' for more information", args[0], metaclient.MetaNodeAddress)
		return
	}
